...
class Metacom extends EventEmitter {
  ...
  async message(msg) {
    ...
    if (!type) return;
    await this[`${type}Handler`](data);
  }

  errorHandler({ callId, message, code }) {
    const promised = this.calls.get(callId);
    if (!promised) return;
    const reject = promised.at(-1);
    this.calls.delete(callId);
    reject(new MetacomError({ message, code }));
  }

  callbackHandler({ callId, result }) {
    const promised = this.calls.get(callId);
    if (!promised) return;
    const [resolve] = promised;
    this.calls.delete(callId);
    resolve(result);
  }

  eventHandler({ iface, eventName, args }) {
    const metacomInterface = this.api[iface];
